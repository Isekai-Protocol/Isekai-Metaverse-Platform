...
class Advertisement extends Common
{
    ...
    public function getAdvertList($code,$page,$limit)
    {
        ...
        $field = 'id,position_id,code,name_china,img,type,val,sort,name_eng';
        $where[] = ['code', 'eq', $code];
        ...
        if(!$list->isEmpty())
        {
            ...
            {
                if(session('lng') == 'eng'){
                    $list[$key]['name_china'] = $val['name_eng'];
                }

                if($code == 'tpl1_slider'){
                    $list[$key]['img'] = _sImage($val['img']).addImageSuffix(1);
                }else{
                    $list[$key]['img'] = _sImage($val['img']).addImageSuffix(5);
                }
