...
class Carousel extends Common
{
    ...
    protected $msg = [
        'name_china.require'              => '请输入广告名称',
        'name_china.max'                  => '广告名最多不能超过50个字符',
        'type.number'               => '广告类型非法',
    ...
    {
        $where = [];
        if (isset($post['name_china']) && $post['name_china'] != "") {
            $where[] = ['name_china', 'like', '%' . $post['name_china'] . '%'];
        }
    ...
    public function getCarouselList($code, $page, $limit)
    {
        ...
        $field   = 'id,name_china,name_eng,img,type,val,sort,code';
        $where[] = ['code', 'eq', $code];
    ...
    public function getCarouselLists($codes)
    {
        ...
        $field   = 'id,position_id,code,name_china,name_eng,img,type,val,sort';
        $where[] = ['code', 'in', $codes];
