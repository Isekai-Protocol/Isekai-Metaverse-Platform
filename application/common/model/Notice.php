...
class Notice extends Common
{
    ...
    {
        $cate = new \app\common\model\HelpCate();
        ...
        if($data){
            foreach ($data as $key => $value) {
                $data[$key]['cate_name'] = $cate->where('id',$value['cate_id'])->value('cate_name');
            }
        }
     ...
     * @throws \think\exception\DbException
     */
    public function getNoticeList($type = 1, $order = 'id', $orderType = 'desc', $page = 1, $pageSize = 10,$cate_id = 0,$keyword = '',$is_recommend = -1)
    {
        $map = [];
        if(!empty($cate_id)){
        ...
        if($is_recommend >= 0){
            $map[] = ['is_hot','eq',$is_recommend];
        }

        $cate = new \app\common\model\HelpCate();

        $list = $this->where($map)->order('sort asc')->page($page, $pageSize)->select();
        // dump($this->getLastSql());die;
        if($list){
            foreach ($list as $key => $value) {
                if(getSetting('language') == 'en'){
                    $list[$key]['title'] = $value['title_eng'];
                }

                if(!empty($value['cate_id'])){
                    $list[$key]['cate_name'] = $cate->where('id',$value['cate_id'])->value('cate_name');
                }else{
                    $list[$key]['cate_name'] = '';
                }
            }
    ...
    public function getNoticeInfo ($id)
    {
        ...
        if ($info) {
            ...
            if(!empty($info['cate_id'])){
                $info['cate_name'] = $cate->where('id',$info['cate_id'])->value('cate_name');
            }else{
                $info['cate_name'] = '';
            }
        }
