...
class Order extends Common
{
    ...
    public function getOrderInfoByOrderID($id, $user_id = false, $aftersale_level = 0)
    {
        ...
        //下单人
        $order_info['username'] = get_user_info($order_info['user_id'],'username');
        ...
        foreach ($order_info->items as $key => $value) {
            if($value['type'] == 1){
                // 文章
                $info = $article->articleDetail($value['goods_id']);
            }elseif($value['type'] == 2){
                // 音频
                $info = $music->getAudioInfo($value['goods_id']);
            }elseif($value['type'] == 3){
                // 游戏
                $info = $game->getGameInfo($value['goods_id']);
            }else{  
                // 插画
                $info = $img->pictureDetail($value['goods_id']);
            }
            
            if($info['status']){
                $order_info->detail = $info['data'];
            }
...
//        $where[] = ['order_type', 'eq', self::ORDER_TYPE_COMMON];
    ...
    // 生成订单
    public function makeOrder($id = '',$user_id = '',$type = 1,$address = '',$buy_type = 0)
    {
        $result = [
            'status' => false,
            'data' => [],
        ...
        // $can_res = canBuyNft($id,$type);
        // if(!$can_res){
        //     return error_code();
        // }
        $buy_res = canBuy($id,$type,$user_id);
        if($buy_res){
        ...
        if($type == 1){
            ...
            $name = $info['title'];
            $cover = $info['cover']; 
            $cate_id = $info['type_id'];
            $code = $info['code'];
            $ower_id = $info['user_id'];
        }elseif($type == 2){
            // 音频
            $info = $musicModel->where('id',$id)->find();
            $name = $info['name'];
            $cover = $info['path'];
            $extend = $info['url'];
            $cate_id = $info['cat_id'];
            ...
            // 游戏
            $info = $gameModel->where('g_id',$id)->find();
            $name = $info['game_name'];
            $cover = $info['cover'];
            ...
            $name = $info['name'];
            $cover = _sImage($goodsExtendModel->where('goods_id',$id)->value('role_img'));
            $cate_id = $info['cate_id'];
            $code = $info['bn'];
        }else{
            // 卡券
            $info = $userCardModel->where('id',$id)->find();
            $info['price'] = 0;
            $info['is_market'] = 0;
            $code = $info['code'];
            $name = '卡券';
        ...
        $order['user_id'] = $user_id;
        $order['order_type'] = 1;
        $order['goods_amount'] = empty($info['price']) ? 0.00 : $info['price'];
        $order['order_amount'] = empty($info['price']) ? 0.00 : $info['price'];
        $order['ctime'] = time();

        // Db::startTrans();
        if(!empty($address)){
            $user_balance = $userWalletModel->where(['address' => $address,'user_id' => $user_id])->value('balance');
            if($user_balance < $order['goods_amount']){
        ...
        try {
            $res_1 = $this->insert($order);
            $itemsData = [
                'order_id' => $order['order_id'],
                'goods_id' => $id,
                'price' => empty($info['price']) ? 0.00 : $info['price'],
                'image_url' => _sImage($cover),
                'nums' => 1,
                'amount' => empty($info['price']) ? 0.00 : $info['price'],
                'type' => $type,
                'sn' => $code,
                'name' => $name,
                'is_market' => 0,
            ...
            $res_2 = $orderItemsModel->save($itemsData);

            if($res_1 && $res_2){
                if($type == 1){
                    // 文章
                    $articleModel->where('id',$id)->update(['is_market' => 0]);
                   
                }elseif($type == 2){
                    // 音频
                    $musicModel->where('id',$id)->update(['is_market' => 0]);
                    
                }elseif($type == 3){
                    // 游戏
                    $info = $gameModel->where('g_id',$id)->update(['is_market' => 0]);
                   
                }elseif($type == 4){
                    // 插画
                    $info = $imgModel->where('id',$id)->update(['is_market' => 0]);
                   
                }elseif($type == 5){
                    // 角色
                    $info = $goodsModel->where('id',$id)->update(['is_market' => 0]);
                ...
                // if(!empty($ower_id)){
                //     $walletInfo = $userWalletModel->where('user_id',$ower_id)->find();
                //     if(!empty($walletInfo)){
                //         mintNFT($ower_id,$id,$type,$user_id,2,$info['price']);
                ...
                // if(empty($address)){
                    // 查询默认钱包
                    // $userWalletModel->where(['user_id' => $user_id])->setDec('balance',$order['goods_amount']);
                // }else{
                    // $userWalletModel->where(['address' => $address,'user_id' => $user_id])->setDec('balance',$order['goods_amount']);
                // } 

                $work_data = $workModel->where(['obj_id' => $id,'type' => $type])->find();
                ...
                if($buy_log_type != 0){
                    addBuyLog($id,$type,$user_id,$buy_log_type,$saleInfo['price'],$address);
                }
                
                //修改成交状态
                $sale_mod = new \app\common\model\UserSale();
                $saleInfo = $sale_mod->where(['obj_id'=>$id,'obj_type'=>$type,'status' => 0])->find();
                if($saleInfo){
                    $sale_res = $sale_mod->where(['obj_id' => $id,'obj_type' => $type])->update(['status' => 1]);
                    if($sale_res){
                        $businessData = [
                            'user_id' => $user_id,
                            'receive_id' => $saleInfo['user_id'],
                        ...
                        // mintNFT($saleInfo['user_id'],$id,$type,$user_id,2,$info['price']);
                    }
                }

                makeMyNft($id,$type,$user_id,2,$work_data,$address,$buy_type);
