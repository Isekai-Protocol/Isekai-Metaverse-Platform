...
class Goods extends Manage
{
    ...
    public function postIndex()
    {
        $goodsModel = new goodsModel();
        $statics    = $goodsModel->staticGoods();
        $this->assign('version_list',array_filter(array_unique($goodsModel->column('post_version'))));
        $this->assign('statics', $statics);
        if (Request::isAjax()) {
            $filter = input('request.');
            return $goodsModel->tableData($filter);
        }
        return $this->fetch('postindex');
    }
    ...
    public function exportPost()
    {
        ...
        $goodsModel = new goodsModel();
        $csvData = $goodsModel->getPostCsv($params);
        // $csvData = [];
        // $csvData = array_values($csvData);
        // dump($csvData);die;

        $title = ['ID','角色名称','职业','性别','编号','版税','角色立绘','特性','称号','历程','天赋','潜能','角色类型','身材','立场','角色详情','表情','组件'];
        $file_name="roles".date("YmdHi",time()).".csv";
    ...
    private function _common()
    {
        ...
        $cateList = $articleTypeModel->where('type',5)->select();
    ...
    public function doAdd()
    {
        ...
        if ($open_spec) {
            ...
            foreach ($product as $key => $val) {
                ...
                $tmp_product['goods']['specs_name'] = isset($val['specs_name']) ? $val['specs_name'] :'';
                $tmp_product['goods']['product_spes'] = $val['specs_type'].':'.$val['specs_name'];
                $tmp_product['goods']['specs_type'] = isset($val['specs_type']) ? $val['specs_type'] : '';
                ...
                $tmp_product['goods']['is_defalut']   = $productsModel::DEFALUT_YES;
    ...
    private function checkGoodsInfo($isEdit = false)
    {
        ...
        $data['goods']['cate_id']  = input('post.cate_id', '0');
        ...
        $data['goods']['work']        = input('post.goods.work', '');
        ...
        $data['goods']['rarity']       = input('post.goods.rarity', '');
        $data['goods']['sex']          = input('post.goods.sex', 0);
        ...
        $data['goods']['is_market']    = input('post.goods.is_market', 0);
        $open_spec                     = input('post.open_spec', 0);
    ...
    private function checkProductInfo($data, $goods_id = 0, $isEdit = false)
    {
        ...
        $data['product']['is_market'] = $data['goods']['is_market'];//是否投放市场
        ...
        if ($open_spec && $data['goods']['product_spes']) {
            ...
            // $data['product']['spes_desc'] = $data['goods']['specs_type'].':'.$data['goods']['specs_name'];
            $data['product']['specs_type'] = $data['goods']['specs_type'];
            $data['product']['specs_name'] = $data['goods']['specs_name'];
        }
        if (!$data['product']['sn']) {
    ...
    private function getEditSpec($type_id, $goods)
    {
        ...
        if ($res['status'] == true) {
            ...
            if ($spes_desc) {
                ...
                foreach ($spes_desc as $key => $value) {
                    foreach ($value as $k => $v) {
                        $items[] = [
                            'spec_name' => $v,
                            'spec_key' => $key,
                ...
                foreach ($goods['products'] as $product) {
                    foreach ($items as $key => $ispec) {
                        if ($ispec['spec_key'].':'.$ispec['spec_name'] == $product['spes_desc']) {
     ...
     * @return mixed
     */
    public function propIndex()
    {
        $id = input('id');
        $propModel = new \app\common\model\Prop();
        if (Request::isAjax()) {
            return $propModel->tableData(input('param.'));
        }
        return $this->fetch('propindex',['goods_id' => $id]);
    }
    ...
    public function propAdd(){
        ...
        if (Request::isPost()) {
            $addData = [
                ...
                'role_img' => input('role_img'),
                'prop_img' => _sImage(input('role_img')).'?x-oss-process=image/crop,x_2086,y_2408,w_842,h_248/format,webp',
                // 'sort' => input('sort')
            ];
            return $propModel->insert($addData) ? ['status' => true,'msg' => '添加成功','data' => ''] : ['status' => false,'msg' => '添加失败'];
            // return $files->addData(input('param.'));
        }

        return $this->fetch('prop_add');
    }
    ...
    public function propEdit(){
       ...
       if (Request::isPost()) {
           ...
           return $propModel->where('id',input('id'))->update($addData) ? ['status' => true,'msg' => '修改成功','data' => ''] : ['status' => false,'msg' => '修改失败'];
           // return $files->addData(input('param.'));
       }

       $info = $propModel->where('id', input('param.id/d'))->find();

       if($info['type'] == 1){
            $info['role_img'] = $info['prop_img'];
       }

       return [
           'status' => true,
           'msg'    => '获取成功',
           'data'   => $this->fetch('prop_edit', ['info' => $info])
       ];
    }
    ...
    {
        $propModel = new \app\common\model\Prop();
        return $propModel->where('id',input('id'))->delete() ? ['status' => true,'msg' => '删除成功','data' => ''] : error_code(10018);
    } 
     ...
     * @return mixed
     */
    public function assemblyIndex()
    {
        $assemblyModel = new \app\common\model\Assembly();
        if (Request::isAjax()) {
            return $assemblyModel->tableData(input('param.'));
        }
        return $this->fetch('assemblyindex');
    }
    ...
    public function setRate()
    {
       ...
       $goodsInfo = $goodsModel->where('id',input('id'))->find();
       if(Request::isPost()){
            return $goodsModel->where('id',input('id'))->update(['royalties' => input('royalties')]) ? ['status' => true,'msg' => '设置成功','data' => ''] : error_code(10004);
       }
       return $this->fetch('',['info' => $goodsInfo]);
    }

    //角色发版
    public function relaseGoods()
    {
        $ids = input('ids');
        $goodsModel = new \app\common\model\Goods();

        $res = $goodsModel->whereIn('id',$ids)->update(['is_post' => 1,'post_version' => input('version')]);

        return $res ? ['status' => true,'msg' => '设置成功','data' => ''] : error_code(10004);
    } 
    ...
    public function editRole()
    {
        ...
        $goodsPartModel = new \app\common\model\GoodsPart();
        $assemblyModel = new \app\common\model\Assembly();
        ...
        if(Request::isPost()){
            if(input('?id')){
                ...
                $goodsModel->where('id',input('id'))->update($goods_data);

                // 特性
                $character_info = [];
                for ($i=0; $i < count(input('character_info.name')); $i++) { 
                    $character_info[] = [
                        'name' => input('character_info.name')[$i],
                        'value' => empty(input('character_info.value')[$i]) ? 0 : input('character_info.value')[$i]
                    ];
                }

                // 称号
                $role_title_info = [];
                for ($m=0; $m < count(input('role_title_info.title')); $m++) { 
                    $role_title_info[] = [
                        'title' => input('role_title_info.title')[$m],
                        'sort' => empty(input('role_title_info.sort')[$m]) ? 0 : input('role_title_info.sort')[$m]
                    ];
                }

                // 天赋
                $talent_info = [];
                for ($j=0; $j < count(input('talent_info.title')); $j++) { 
                    $talent_info[] = [
                        'title' => input('talent_info.title')[$j],
                        'img' => empty(input('talent_info.img')[$j]) ? _sImage() : input('talent_info.img')[$j]
                    ];
                }

                // 潜能
                $potential_info = [];
                for ($k=0; $k < count(input('potential_info.title')); $k++) { 
                    $potential_info[] = [
                        'title' => input('potential_info.title')[$k],
                        'img' => empty(input('potential_info.img')[$k]) ? _sImage() : input('potential_info.img')[$k]
                    ];
                }

                $extend_data = [
                    'work_type' => input('work_type'),
                    'work_life' => input('work_life'),
                    'history' => input('history'),
                    'role_img' => input('role_img'),
                    'position' => input('position'),
                    'role_title_info' => serialize($role_title_info),
                    'character_info' => serialize($character_info),
                    'talent_info' => serialize($talent_info),
                    'potential_info' => serialize($potential_info),
                    'role_type' => input('role_type'),
                    'figure' => input('figure')
                ];

                $res = $goodsExtendModel->where('goods_id',input('id'))->update($extend_data);

                return ['status' => true,'msg' => '成功'];
            }
        }else{
            $info['info'] = $goodsModel->field('id,name,work,sex,name_eng,royalties')->where('id',input('id'))->find();//角色基本信息
            $extend = $goodsExtendModel->where('goods_id',input('id'))->find();
            if($extend){
                $info['extend'] = $extend;
                $info['extend']['role_title_info'] = unserialize($extend['role_title_info']);
                $info['extend']['character_info'] = unserialize($extend['character_info']);
                $info['extend']['talent_info'] = unserialize($extend['talent_info']);
                $info['extend']['potential_info'] = unserialize($extend['potential_info']);
                $part_ids = $goodsPartModel->where('goods_id',input('id'))->column('part_id');
                $info['part_arr'] = [];
                if(!empty($part_ids)){
                    $info['part_arr'] = $assemblyModel->whereIn('id',$part_ids)->select();
                }

            }

            return $this->fetch('new_role',['info' => $info]);
        }
    ...
    {
        $scriptModel = new \app\common\model\Script();
        if (Request::isAjax()) {
            return $scriptModel->tableData(input('param.'));
        }
        return $this->fetch('unity');
    }
}
